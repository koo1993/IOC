...
#water mark info from https://github.com/DidierStevens/DidierStevensSuite/blob/master/1768.json
watermark_info = json.load(open('./watermark.json', 'r'))
watermark_info = watermark_info["dLookupValues"]["37"]
shortened_json = []
...
for eachscan in data:
    ...
    #get ip location from maxmind
    maxmind_ip_loc = get_maxmind_ip_information(ip)
    eachscan['country_maxmind'] = maxmind_ip_loc

    #get watermark info
    eachscan['watermark_info'] = "None"
    if str(eachscan["watermark"]) in watermark_info:
        eachscan['watermark_info'] = watermark_info[str(eachscan["watermark"])]
    ...
    if ip_source.__eq__(""):
        ip_source = ip_source + "SGIPTracker.txt"
    eachscan['ip_source'] = ip_source

    #pick element for faster reference
    element = {}
...
print("shorterned version in result_shorten.json")
